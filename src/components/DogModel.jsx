...
const DogModel = (props) => {
  ...
  const handleClick = () => {
    ...
    // Filter out 'standing_skeletal.3' and 'rollover_skeletal.3'
    const nonIdle = Object.keys(actions).filter(
      (name) => name !== 'standing_skeletal.3' && name !== 'rollover_skeletal.3'
    );
    ...
    const onFinished = (e) => {
      if (e.action === action) {
        ...
        const idleAction = actions['standing_skeletal.3'];
        // Reset and play the idle animation *after* the current frame has finished.
        // This ensures the mixer is in a stable state.
